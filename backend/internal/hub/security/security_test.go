...
func TestPasswordArgon2idRoundTrip(t *testing.T) {
	...
	if !security.VerifyPassword(hash, "correct horse battery staple") || security.VerifyPassword(hash, "wrong password") {
...
func TestAccessJWTClaimsAndTTL(t *testing.T) {
	...
	if claims.Subject == "" || claims.DeviceID == "" || claims.SessionID == "" {
...
func TestCSRFIsBoundToOpaqueSessionSecret(t *testing.T) {
	...
	if !security.VerifyCSRF("opaque-cookie", token, key) || security.VerifyCSRF("other-cookie", token, key) {
