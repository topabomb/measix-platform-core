...
type Config struct{ PublicListenAddr, InternalListenAddr, SpoolPath string }

func Load(args []string) (Config, error) {
	fs := flag.NewFlagSet("runtime-relay", flag.ContinueOnError)
	cfg := Config{PublicListenAddr: env("RELAY_PUBLIC_LISTEN_ADDR", ":8090"), InternalListenAddr: env("RELAY_INTERNAL_LISTEN_ADDR", "127.0.0.1:8091"), SpoolPath: env("RELAY_SPOOL_PATH", "relay-spool.db")}
