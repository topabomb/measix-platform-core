...
It also owns executable API contracts, the current database initialization schema, qualification/system-test infrastructure, CI and operational procedures needed to prove those components satisfy architecture.
...
| Ent schema / current Atlas initialization | repository source |
...
Control Hub persistent application state uses SQLite + Ent with one reviewed Atlas initialization schema. Runtime Relay persists only its own durable runtime data such as usage spool and does not share Hub ORM state.

This repository is authoritative for actual Ent schemas, the current initialization SQL/`atlas.sum`, clean bootstrap and schema-identity tests. Architecture remains authoritative for durability, ownership, atomicity and recovery invariants.
