...
| 阿里云百炼新加坡 Token Plan | 用户指定的 `token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1` 直连：`qwen3.8-flash` 对话、工具调用及后续回合、图片理解、`qwen-audio-3.0-tts-plus` 音频、`qwen-audio-3.0-asr-flash` 文件识别均成功；`qwen-image-3.0-pro` 通过百炼原生同步接口生图成功。工具具名强制调用必须传 `enable_thinking=false`；默认思考模式下返回 400。2026-09-21 复核时，该 Token Plan 域名的 `/compatible-mode/v1/images/generations` 返回空 body 的 HTTP 404，不能把原生接口成功等同于 OpenAI Images profile 成功 | 用户明确本机仅做实验，已将密钥保存到此开发 Hub 的 Secret Store 并建立实验 Upstream。Core/Android 当前已支持独立 `OPENAI_IMAGES_GENERATIONS` 托管资源；但这组 Token Plan 配置未通过该 profile 资格，因此当前真实预设不发布图片资源。需提供能响应 OpenAI Images 的业务空间专属域名或其他兼容供应商后再完成托管链路验收 |
...
公共入口 `http://192.168.31.235:9100` 同时承载 Discovery、Client API、Runtime Relay、Portal 和 Admin；手机在同一局域网可使用 HTTP IP 地址。当前真实预设下发 DeepSeek Flash 和 Qwen 3.8 Flash 两个 TEXT/IMAGE/TOOL 模型、MiMo 云端朗读与设备系统朗读、百炼 HTTP 录音转写、Firecrawl Direct MCP、两个企业助手和三个常用入口。`policy.defaultModelId` 指向 DeepSeek，`defaultTtsId` 指向 MiMo，`defaultAsrId` 指向百炼，`defaultAssistantId` 指向企业工作助手；`defaultImageGenerationId` 当前未设置。五项 `allowLocal*` 允许在企业域按策略混用个人资源。Qwen 和百炼 ASR 标注“本机实验”；图片托管 profile 已实现，但现有 Token Plan 地址未通过 OpenAI Images 路径资格，故没有伪装成可用的 `img_*` 资源。
