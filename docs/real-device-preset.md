...
已发布资源为：DeepSeek Flash、Qwen 3.8 Flash（本机实验）、MiMo 云端朗读、设备本地朗读、百炼 HTTP 录音转写（本机实验）、Firecrawl Streamable HTTP MCP、两个企业助手和三个常用入口。默认项为 DeepSeek、MiMo、百炼 ASR 和企业工作助手。所有上游目前为 `LEVEL_0`：管理台可验证转发次数、状态和字节数，但不能把供应商 token 或费用显示为已知。
