...
| Ent schema / current initialization SQL | repository source |
...
Control Hub persistent application state uses SQLite + Ent with one reviewed initialization SQL. Runtime Relay persists only its own durable runtime data such as usage spool and does not share Hub ORM state.

This repository is authoritative for actual Ent schemas, the current initialization SQL, clean bootstrap and schema-identity tests. Architecture remains authoritative for durability, ownership, atomicity and recovery invariants.
